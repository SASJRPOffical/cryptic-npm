...
async function postBan(userId, userTag, databaseName, banReason, banProof, enforcerId, httpRoute) {
...
async function getGlobalBans(httpRoute) {
    if (typeof token.get() == 'undefined') return console.log('[Cryptic Core] Firewall: requires a token to post a ban, use token.set("YOUR_API_TOKEN") before utilisiing getGlobalBans.');
    // if (typeof providedToken == 'undefined') return 
    if (typeof host.get() == 'undefined') return console.log('[Cryptic Core] Firewall: no API Host was provided. firewall requires a Host Url to post a ban, use host.set("YOUR_API_HOST") before utilisiing getGlobalBans.');
    if (typeof httpRoute == 'undefined') return console.log('[Cryptic Core] Firewall: no API Route was provided');
...
async function deleteBan(banId, reason, databaseName, staff, extras, httpRoute) {
    if (typeof token.get() == 'undefined') return console.log('[Cryptic Core] Firewall: requires a token to post a ban, use token.set("YOUR_API_TOKEN") before utilisiing deleteBan.');
    if (typeof host.get() == 'undefined') return console.log('[Cryptic Core] Firewall: no API Host was provided. firewall requires a Host Url to post a ban, use host.set("YOUR_API_HOST") before utilisiing deleteBan.');
    if (typeof httpRoute == 'undefined') return console.log('[Cryptic Core] Firewall: no API Route was provided');
